<template>
  <div class="m-4 mr-0 overflow-hidden bg-white" v-bind:style="{'max-width':'230px'}">
    <div class="full_btn">
    <div class="zTreeBackground">
        <ul id="treeData" class="ztree">
        </ul>
         <!--右键菜单-->
         <div id="rMenu" v-show="showRightNemu">
            <ul class="menubox">
                <li class="menuItem">
                  <span @click="addGroup">
...
<script lang="ts">
  ...
  import { getGroupList } from '/@/api/microweb/webmain';
  import { upGrouppid,delData } from '/@/api/microweb/group';
    ...
    setup(_, { emit }) {
      ...
      async function fetch() {
       let zNodes=[
        ...
        const treeList= (await getGroupList()) as unknown as TreeItem[];
      ...
      function showRMenu( x, y) {
          $("#rMenu").css({"top": (y-100) + "px", "left": (x-50) + "px", "visibility": "visible"}); //设置右键菜单的位置、可见
